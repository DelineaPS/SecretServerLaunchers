...
Alternate [IBM Access Client Solutions](https://www.ibm.com/support/pages/ibm-i-access-client-solutions) launcher that takes a separate `IPADDR` parameter — useful when the system hostname differs from how clients should resolve it (e.g. via a load balancer VIP).

## Bundled batch file

[`launch-acs-positional.bat`](launch-acs-positional.bat). Replace `<loc>` in the script with the full path to the IBMiAccess install directory (e.g. `C:\Users\Public\IBM\ClientSolutions\`) before uploading.
